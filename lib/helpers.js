...
export function fastHash({
  clientId, hash, data, encoding, prefix = false
} = {}) {
  if(data === undefined) {
    if(clientId) {
      if(!hash) {
        throw new TypeError(
          '"hash" must be provided when providing "clientId".');
      }
      data = `${hash}:${clientId}`;
    } else {
      data = hash;
...
export function verifyHash({clientId, hash, sha256} = {}) {
  ...
  return crypto.timingSafeEqual(sha256, fastHash({clientId, hash, prefix}));
}
