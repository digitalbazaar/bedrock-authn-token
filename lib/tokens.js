 ...
 * @param {string} [options.hashAlgorithm='bcrypt'] - The hash algorithm used
 *   to compute the hashed password (only applicable to `type === password`).
...
export async function set({
  ...
  if(type !== 'nonce') {
    ...
    if(type === 'password') {
      assert.string(hashAlgorithm, 'hashAlgorithm');
    }
  }
  ...
  const result = await _createToken({
    ...
    hash, hashAlgorithm, clientId, serviceId, typeOptions
  });
...
export async function get({
  ...
  // add default `hashAlgorithm` as needed
  if(type !== 'totp' && !token.hashAlgorithm) {
    token = {...token, hashAlgorithm: 'bcrypt'};
  }
...
export async function verify({
  ...
  let token;
  if(type === 'nonce') {
    token = _getMatchingNonce({record, clientId, hash});
    if(!token) {
  ...
  if(type === 'nonce') {
    // a matching nonce token was found above by verifying its hash
    verified = true;
    ...
    // verify hashed password in `hash` against hash-based token value
...
async function _addPasswordToken({
  accountId, email, token, hash, hashAlgorithm = 'bcrypt'
}) {
...
async function _createToken({
  ...
  if(type === 'nonce') {
    ...
    result = _addPasswordToken({accountId, email, token, hash, hashAlgorithm});
  } else if(type === 'totp') {
...
function _getMatchingNonce({record, clientId, hash}) {
  const tokens = record.meta['bedrock-authn-token'].tokens.nonce;
  const now = new Date();
  for(const token of tokens) {
    if(!verifyHash({clientId, hash, sha256: token.sha256})) {
      continue;
    }
