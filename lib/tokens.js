...
export async function set({
  ...
  if(type === 'password') {
    await _setupPasswordToken({token, hash});
  } else if(type === 'nonce') {
    const {challenge} = await _setupNonceToken(
      {token, accountId, email, clientId, typeOptions});
  ...
  // set / push token in database
  const result = await (type === 'nonce' ?
    _pushNonceToken({accountId, email, token, explain}) :
    _setToken({accountId, email, type, token, explain}));
  if(explain) {
...
async function _setupPasswordToken({token, hash}) {
...
async function _setupNonceToken({
  token, accountId, email, clientId, typeOptions
}) {
  const cfg = config['authn-token'];
