...
export async function get({
  ...
  // add default `hashParameters` as needed
 ...
 *   `nonce`, `totp`).
 * @param {string} [options.hash] - The slow hash (e.g., pbkdf2 or bcrypt) to
 *   verify for a password/nonce; `bcrypt` is deprecated.
...
async function _addNonceToken({
  ...
  // reuse hash params from the last token if they are up to date with the
  // system configuration
  let hashParameters;
  const lastToken = tokens[tokens.length - 1];
  if(lastToken?.hashParameters?.id === cfg.pbkdf2.id &&
    lastToken?.hashParameters?.params?.i === cfg.pbkdf2.iterations) {
    hashParameters = lastToken.hashParameters;
  } else {
  ...
  // FIXME: do not slow hash large random challenges
  ...
  token.sha256 = fastHash({clientId, hash});
