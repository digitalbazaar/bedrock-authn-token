...
export async function set({
  ...
  const result = await _createToken({
    ...
    hash, clientId, serviceId, typeOptions, explain
  });
...
// FIXME: export for testing and move into database helpers file
async function _getAccountRecord({
...
async function _setToken({accountId, email, type, token, explain}) {
  const updated = await _updateTokenValue({
    accountId, email, type, value: token, op: '$set', explain
  });
  if(explain) {
    // return explain result
    return updated;
...
async function _pushNonceToken({accountId, email, token, explain}) {
  const {maxNonceCount} = config['authn-token'].nonce;
  const type = 'nonce';

  const maxIndex = maxNonceCount - 1;
  const query = {
    [`meta.bedrock-authn-token.tokens.nonce.${maxIndex}`]: {$exists: false}
  };

  // keep trying to push nonce token until success or error
  let updated;
  while(!updated) {
    updated = await _updateTokenValue({
      accountId, email, query, type, value: token, op: '$push', explain
    });
    if(explain) {
      // return explain result
      return updated;
...
async function _addNonceToken({
  accountId, email, token, clientId, typeOptions, explain
}) {
  const cfg = config['authn-token'];
  ...
  // add token
  const result = await _pushNonceToken({accountId, email, token, explain});
  if(explain) {
    // return explain result
    return result;
  }
...
async function _addPasswordToken({accountId, email, token, hash, explain}) {
  ...
  // add token
  const result = await _setToken(
    {accountId, email, type: 'password', token, explain});
  if(explain) {
    // return explain result
    return result;
  }
...
async function _addTotpToken({accountId, email, token, serviceId, explain}) {
  ...
  // add token
  const result = await _setToken(
    {accountId, email, type: 'totp', token, explain});
  if(explain) {
    // return explain result
    return result;
  }
...
// FIXME: move into database helpers file
async function _updateTokenValue({
  accountId, email, query, type, value, op, explain
}) {
  ...
  // set token value in account `meta`
  const result = await database.collections.account.updateOne(
    query, {
      [op]: {
        [`meta.bedrock-authn-token.tokens.${type}`]: value
      }
    });
...
async function _createToken({
  ...
  if(type === 'nonce') {
    ...
    result = _addPasswordToken({accountId, email, token, hash, explain});
  } else if(type === 'totp') {
    result = _addTotpToken({accountId, email, token, serviceId, explain});
  }
