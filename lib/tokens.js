...
async function _addNonceToken({
  ...
  // generate new challenge; if an `email` was provided that is marked as
  // a tester email, then always generate the same challenge token
...
function _isTesterAccount({accountId, email}) {
  ...
  const match = testerAccounts.find(
    e => e.id === accountId || e.email === email);
