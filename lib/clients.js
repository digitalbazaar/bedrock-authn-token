 ...
 * @param {string} [options.account] - The ID of the account to add a client
 *   for.
 ...
 */
api.set = async ({
  account, email, clientId, authenticated, notify = true, explain = false
} = {}) => {
  ...
  if(authenticated) {
    ...
    const {registered} = await api.isRegistered({account, email, clientId});
    if(registered) {
 ...
 */
api.isRegistered = async ({
  account, email, clientId, explain = false
} = {}) => {
  assert.optionalString(account, 'account');
  assert.optionalString(email, 'email');
  if(!(account || email)) {
