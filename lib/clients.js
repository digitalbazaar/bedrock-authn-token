...
export async function set({
  ...
  // FIXME: consider changing format to store clients as an array instead;
  // to allow for easier enforcement of client count, limit will likely be
  // 5-10, don't need to optimize for look ups based on ID

  const key = fastHash({data: clientId, encoding: 'base64'});
  const client = {
    id: key,
    // FIXME: could expire clients; could store last authentication time
    // to use it to assist w/auto-expiring clients when they generate too
    // many concurrent nonce tokens?
    authenticated
  };
