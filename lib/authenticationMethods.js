...
bedrock.events.on('bedrock-account.insert', ({meta}) => {
  ...
  let authnTokenMeta = meta['authn-token'];
  if(!authnTokenMeta) {
    authnTokenMeta = meta['authn-token'] = {};
  }
  if(!authnTokenMeta.requiredAuthenticationMethods) {
    authnTokenMeta.requiredAuthenticationMethods =
