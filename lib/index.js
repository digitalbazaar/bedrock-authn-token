...
api.set = async ({
  ...
  if(type !== 'nonce') {
    if(!account) {
      throw new Error('"account" must be given.');
    }
  ...
  if(type === 'nonce') {
    ...
    const oldValue = tokens.length > 0 ? tokens.slice() : {$in: [null, []]};
...
api.get = async ({
  ...
  if(filterExpiredTokens) {
    const now = Date.now();
    if(Date.parse(rValue.expires) < now) {
      // throw NotFoundError
      throw new BedrockError(
...
api.getAll = async ({
  ...
  if(filterExpiredTokens) {
    result = result.filter(token => {
      return Date.parse(token.expires) > now;
    });
  }
  return result;
};
...
api.verify = async ({
  ...
  if(type === 'nonce') {
    for(const token of tokenValue) {
      if(type === 'nonce') {
        if(token.sha256 === prefixedHash(hash)) {
    ...
    if(tokenValue.expires && new Date() >= new Date(tokenValue.expires)) {
      throw new BedrockError(
        'Authentication token has expired.',
        'NotFoundError', {
          httpStatusCode: 404,
