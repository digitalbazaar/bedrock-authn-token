...
api.set = async ({
  ...
  if(type === 'password') {
    ...
    try {
      ...
      tokens = await api.getAll({
        actor: null, account, email, type, filterExpiredTokens: true
      });
    } catch(e) {
    ...
    if(tokens.filteredTokens.length >= maxNonceCount) {
  ...
  if(type === 'nonce') {
    ...
    const oldValue = tokens.allTokens.length > 0 ? tokens.allTokens
      .slice() : {$in: [null, []]};
    query['meta.bedrock-authn-token.tokens.nonce'] = oldValue;
    tokens.filteredTokens.push(token);
    tokenTypeValue = tokens.filteredTokens;
  } else {
