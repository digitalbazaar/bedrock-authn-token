...
api.set = brCallbackify(async ({actor, account, email, type, hash}) => {
  ...
  if(type === 'password') {
    ...
    //   and nonce, produce bcrypt for the nonce,
    //   prefixedHash(bcrypt(salt, nonce)) and then store it
...
api.verify = brCallbackify(async ({actor, account, email, type, hash}) => {
  ...
  if(token.expires) {
    if(new Date() >= new Date(token.expires)) {
      throw new BedrockError(
        'Authentication token has expired.', 'NotFoundError', {
        httpStatusCode: 404,
        public: true
      });
    }
  ...
  // FIXME: remove this `type` check hack to temporarily disable non-password
  // token auth
  const verified = (type !== 'password') || crypto.timingSafeEqual(
