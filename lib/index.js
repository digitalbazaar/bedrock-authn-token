 ...
 * @param [clientId] an identifier for the client to bind a token to, if
 *   supported by the token type.
 * @param [hash] the bcrypt hash to use to set a password/nonce or other
 *   type of token that uses hashed token values.
 * @param [authenticationMethod=type] an optional, application specific
 *   authentication method identifier to assign to this token, useful for
 *   multifactor flows to create dependencies on different types or instances
 *   of tokens.
 * @param [requiredAuthenticationMethods=[]] an optional, application specific
 *   set of authentication methods (specified by ID) that must first be
 *   satisfied before this token can be verified.
 * @param [serviceId] an identifier to associate with the token. For TOTP
 *   tokens, `serviceId` is encoded in the `otpAuthUrl` value returned
 *   by this API.
...
api.set = async ({
  ...
  token.id = base58.encode(await randomBytes(16));
 ...
 * @param [hash] the bcrypt hash to verify for a password/nonce or other
 *   type of token that uses hashed token challenges.
 * @param [challenge] the token challenge value for token types that do not
 ...
 *
 * @return a Promise that resolves to an object containing the account ID,
 ...
 * @param account the ID of the account.
 *
 * @return a Promise that resolves to {requiredAuthenticationMethods}.
 */
 ...
 *
 * @param actor the actor or capabilities to perform the action.
 * @param account the ID of the account to set the recovery email for.
 * @param recoveryEmail the recovery email to set.
 * @param [notify=true] `true` to notify the account user, `false` not to.
 *
 * @return a Promise that resolves once the operation completes.
 */
