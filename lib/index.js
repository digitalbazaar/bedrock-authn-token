 ...
 *   user, `false` not to.
 * @param {string} [options.typeOptions={entryStyle: 'human', nonceLength: 9}] -
 *   Setting entryStyle to `human` will generate default 9 character
 *   numeric-only nonce and setting it to `machine` will generate base58 encoded
 *   nonce. `nonceLength` controls the length of the `human` nonce.
...
api.set = async ({
  ...
  notify = true, typeOptions = {entryStyle: 'human', nonceLength: 9},
  explain = false
} = {}) => {
