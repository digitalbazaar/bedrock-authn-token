...
exports.notify = require('./notify').notify;
 ...
 * @param {string} [options.accountId] - The ID of the account to set a token for.
 ...
 */
exports.set = async ({
  account, email, type, clientId, serviceId, hash,
  ...
  if(type === 'password') {
    ...
    ({allTokens, tokens} = await exports.getAll({account, email, type}));
    ...
    try {
      await exports.get({account, email, type});
    ...
    if(!email) {
      const record = await brAccount.get({id: account});
  ...
  if(notify) {
    try {
      await exports.notify({
        account, email, authenticationMethod,
 ...
 */
exports.get = async ({
  account, email, type, id, filterExpiredTokens = true, explain = false
} = {}) => {
  assert.optionalString(account, 'account');
  assert.optionalString(email, 'email');
 ...
 */
exports.getAll = async ({account, email, type, explain = false} = {}) => {
  assert.optionalString(account, 'account');
  assert.optionalString(email, 'email');
 ...
 */
exports.remove = async ({account, type, id, explain = false} = {}) => {
  assert.string(account, 'account');
  assert.optionalString(id, 'id');
  validateTokenType(type);
...
exports.verify = async ({
  account, email, type, hash, challenge, clientId,
  ...
  // TODO: could call `exports.get()` here if not for the need for
  ...
  if(requiredAuthenticationMethods) {
    const met = await exports.checkAuthenticationRequirements({
      requiredAuthenticationMethods,
  ...
  if(verified) {
    ...
    if(type === 'nonce') {
      try {
        await exports.remove({account: record.account.id, type});
      } catch(e) {
 ...
 *   the operation completes or an ExplainObject if `explain=true`.
 */
exports.setAuthenticationRequirements = async ({
  account, requiredAuthenticationMethods, explain = false
} = {}) => {
  assert.string(account, 'account');
  assert.array(requiredAuthenticationMethods, 'requiredAuthenticationMethods');

  // add token to meta
  const query = {id: database.hash(account)};
  ...
  if(result.result.n === 0) {
    ...
    if(!await brAccount.exists({id: account})) {
      throw new BedrockError(
 ...
 *   {requiredAuthenticationMethods}.
 */
exports.getAuthenticationRequirements = async ({account}) => {
  assert.string(account, 'account');

  const {meta} = await brAccount.get({id: account});
 ...
 *   `false` if not.
 */
exports.checkAuthenticationRequirements = async ({
 ...
 */
exports.setRecoveryEmail = async ({
  account, recoveryEmail, notify = true
} = {}) => {
  assert.string(account, 'account');
