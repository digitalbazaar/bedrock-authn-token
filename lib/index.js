...
api.set = async ({
  ...
  if(type === 'password') {
    ...
    // if there is an existing nonce, delete it, and generate a new one
    let existingToken;
    try {
      // check to see if nonce exists
      existingToken = await api.get({actor: null, account, email, type});
    } catch(e) {
    ...
    if(existingToken) {
      try {
        // the `get` API call did not throw, a nonce exists, delete it
        await api.remove({actor, account, type});
      } catch(e) {
        if(e.name !== 'NotFoundError') {
          throw e;
        }
