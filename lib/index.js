...
api.set = async ({
  ...
  if(type === 'password') {
    ...
    ({allTokens, tokens} = await api.getAll({account, email, type}));
    ...
    let err;
    try {
      await api.get({account, email, type});
    } catch(e) {
 ...
 */
api.getAll = async ({account, email, type, explain = false} = {}) => {
  assert.optionalString(account, 'account');
  assert.optionalString(email, 'email');
 ...
 */
api.remove = async ({account, type, id, explain = false} = {}) => {
  assert.string(account, 'account');
  assert.optionalString(id, 'id');
  validateTokenType(type);
...
api.verify = async ({
  ...
  if(verified) {
    ...
    if(type === 'nonce') {
      try {
        await api.remove({account: record.account.id, type});
      } catch(e) {
 ...
 *   {requiredAuthenticationMethods}.
 */
api.getAuthenticationRequirements = async ({account}) => {
  assert.string(account, 'account');
