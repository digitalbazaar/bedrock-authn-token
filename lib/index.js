...
exports.set = async ({
  ...
  if(type === 'password') {
    ...
    ({allTokens, tokens} = await exports.getAll({accountId, email, type}));
    ...
    try {
      await exports.get({accountId, type});
 ...
 */
exports.getAll = async ({accountId, email, type, explain = false} = {}) => {
  assert.optionalString(accountId, 'accountId');
  assert.optionalString(email, 'email');
  if(!(accountId || email)) {
    throw new Error('Either "accountId" or "email" is required.');
 ...
 */
exports.remove = async ({accountId, type, id, explain = false} = {}) => {
  assert.string(accountId, 'accountId');
  assert.optionalString(id, 'id');
  validateTokenType(type);
 ...
 *   {requiredAuthenticationMethods}.
 */
exports.getAuthenticationRequirements = async ({accountId} = {}) => {
  assert.string(accountId, 'accountId');
