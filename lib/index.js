...
api.set = async ({
  ...
  if(type === 'password') {
    ...
    try {
      ...
      existingTokens = await api.getAll({actor: null, account, email, type});
    } catch(e) {
    ...
    if(existingTokens) {
      ...
      for(const existingToken of existingTokens) {
        ...
        if(existingToken && now > new Date(existingToken.expires)) {
          try {
            // the `nonce` has expired, delete it
            await api.remove({actor, account, type, tokenId: existingToken.id});
            removeCount++;
          } catch(e) {
            if(e.name !== 'NotFoundError') {
              throw e;
 ...
 */
api.get = async ({actor, account, email, type, tokenId} = {}) => {
  assert.optionalString(account, 'account');
  assert.optionalString(email, 'email');
  assert.optionalString(tokenId, 'tokenId');
  if(!(account || email)) {
    throw new Error('Either "account" or "email" must be provided.');
  }
  validateTokenType(type);
  ...
  if(type === 'nonce') {
    if(tokenId) {
      return result.filter(token => token.id === tokenId)[0];
    } else {
      return result[0];
    }
  }
 ...
 * @param type the type of tokens to get.
 *
 * @return a Promise that resolves to tokens.
 */
api.getAll = async ({actor, account, email, type} = {}) => {
  assert.optionalString(account, 'account');
  assert.optionalString(email, 'email');
  if(!(account || email)) {
    throw new Error('Either "account" or "email" must be provided.');
 ...
 */
api.remove = async ({actor, account, type, tokenId} = {}) => {
  ...
  let result;
  if(type === 'nonce') {
    result = await database.collections.account.updateOne({
      id: database.hash(account),
    }, {
      $pull: {
        [`meta.bedrock-authn-token.tokens.${type}`]: {id: tokenId}
      }
    }, database.writeOptions);
  } else {
    result = await database.collections.account.updateOne({
