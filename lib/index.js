...
api.set = async ({
  ...
  if(type === 'password') {
    ...
    let combinedHash;
    if(clientId) {
      combinedHash = prefixedHash(`${hash}:${clientId}`);
    } else {
      combinedHash = prefixedHash(hash);
    }
    token.sha256 = combinedHash;
...
api.verify = async ({
  ...
  if(type === 'nonce') {
    for(const token of tokenValue) {
      nonceVerified = crypto.timingSafeEqual(
        new Buffer(token.sha256, 'base64'),
        new Buffer(combinedHash, 'base64'));
      if(nonceVerified) {
