 ...
 *   user, `false` not to.
 * @param {string} [options.typeOptions='human'] - Setting typeOptions to
 *   `human` will generate 9 character numeric-only nonce and setting it to
 *   `machine` will generate 128-bit bnid.
 *
...
api.set = async ({
  ...
  if(type === 'password') {
    ...
    // generate new token
    challenge = await generateNonce({entryStyle: typeOptions});
