 ...
 *   supported by the token type. For TOTP tokens, `clientId` is encoded
 *   in the `otpAuthUrl` value returned by this API.
...
api.set = async ({
  ...
  if(secret) {
    ...
    rValue.otpAuthUrl = authenticator.keyuri(
      // user identifier
      email || account,
      // a service name, `undefined` is OK if `clientId` was not passed in
      clientId,
      secret
    );
  }
